...
namespace Midterm
{
    class VendingMachine : IEnumerable
    {
        ...
        //I'm not sure why, but it always crashes right here with an stack overflow exemption.
        //Nothing online or in the book is any help with this, so here's where i have to stop.
        //The other solutions should also work, assuming this portion would work.
         VendingMachineOption[,] options = new VendingMachineOption[5,4];

        public VendingMachine()
        {
            //Tried adding each to the location in the array, but this worked better with the enumeration.   
        ...
        //outputs an enumerable in other classes calls.
        public VendingMachineOption this[int x, int y]
        {
            //set { options2 =  (options[x, y]); }
            get { return options[x, y]; }
        }

        //Changed to an enumerable to match the variables.
        public void PrintVendingMachine(List<VendingMachineOption> exclusions)
        {
            ...
            for (int row = 0; row < 5; row++)
            {
                ...
                {
                    Console.WriteLine(exclusions.Contains(options[row, col]) ? FormattedStringToFifteenCharCentered("X") : FormattedStringToFifteenCharCentered(options[row, col].ToString()));
                }
                Console.Write("|");
                Console.WriteLine();
            }
        ...
        public IEnumerator GetEnumerator()
        {
            ...
            foreach ( var Opt1 in options) {
                yield return Opt1.Cast<IEnumerable>();
            }
        }
